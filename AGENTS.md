...
- **Backend/web code reviews AND writing backend/web code → MUST follow [docs/CODE_REVIEW_STANDARD.md](docs/CODE_REVIEW_STANDARD.md)** (the production-grade review prompt: per-item PASS/FAIL/FIXED/NEEDS CONTEXT/NOT APPLICABLE, autonomous-fix mode, full output format). Apply it every time, not just when asked.
- **Completion bar — do NOT declare backend/web work done until ALL hold:** `tsc --noEmit` = 0, all test suites pass (new code has tests), build succeeds, and every safe finding is FIXED (not deferred). The only allowed deferrals are schema / auth-flow / payment / data-migration changes that need approval — surface those explicitly as NEEDS CONTEXT, never bury them. "Everything passed and completely solved before completion."
