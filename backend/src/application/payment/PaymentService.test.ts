...
jest.mock('../../infrastructure/database/prisma', () => ({
  prisma: {
    payment: { findUnique: jest.fn(), findUniqueOrThrow: jest.fn(), findFirst: jest.fn(), update: jest.fn(), updateMany: jest.fn() },
...
const mockedPrisma = prisma as unknown as {
  payment: { findUnique: jest.Mock; findUniqueOrThrow: jest.Mock; findFirst: jest.Mock; update: jest.Mock; updateMany: jest.Mock };
...
function makeTx(over: Partial<Record<string, unknown>> = {}): Tx {
  return {
    ...
    booking: { findUnique: jest.fn().mockResolvedValue({ technicianId: 'tp1' }), update: jest.fn().mockResolvedValue({}) },
    ...over,
  } as Tx;
}
