...
| **Materials never priced by the technician alone** — catalogue-priced, capped, ops-reviewed above threshold | `material_catalog` price book + `quote_lines` + §17.5.8 controls | **Ships with the first `quote_first` category** |
...
### 0.4b Revenue model (all streams, phased — v1.8)
...
### 0.6 Delivery model & operating constraints (v1.9 — the founder-operator reality)
...
```
FEATURE_QUOTE_FIRST=false        # quote engine + materials (needs the readiness gate, §17.5.6)
...
FEATURE_DIRECT_SUPPLY=false      # Fixly Direct Supply for materials (§17.5.10)
```
...
| **Month 7+** | Open `quote_first` **only if the readiness gate passes** (§17.5.6); then widen category or district | Founder + first part-time ops hire |
...
>
> 1. **Nothing that requires buying inventory, pre-paying suppliers, or signing a supply contract.** This is why the materials model is **Asset-Light** (§17.5.10) — Fixly is a *transparency and verification layer over the market that already exists*, not a materials trader.
...
CREATE TYPE materials_policy AS ENUM ('labour_only','materials_included','materials_quoted');
CREATE TYPE quote_line_kind  AS ENUM ('labour','material','prep','fee');
CREATE TYPE material_tier    AS ENUM ('economy','standard','premium');
CREATE TYPE supplied_by      AS ENUM ('customer','technician','platform');
...
-- WHO procured the material. 'platform_arranged' is the L3/Fixly-Direct-Supply path (§17.5.10)
...
-- "BOM frozen once work starts" rule (§17.5.12); 'replaced'/'unused' carry substitution + surplus.
...
-- v1.10 price-book maintenance (§17.5.14). Categories are graded by the RELIABILITY of the
...
-- v1.10 variance dispute protocol (§17.5.15)
...
CREATE TABLE services (
  ...
  -- v1.7 coarse flag, SUPERSEDED by service_material_policies.mode (5-class material_mode).
  -- Kept for the seed/backfill path only; new logic must read service_material_policies (§17.5.4).
  materials_policy    materials_policy NOT NULL DEFAULT 'labour_only',  -- deprecated: see service_material_policies
...
CREATE TABLE booking_quotes (
  ...
  -- non-catalogue material line) require ops approval before they are sent (§17.5.8)
...
CREATE TABLE quote_lines (
  ...
  supplied_by    supplied_by NOT NULL DEFAULT 'technician', -- who sources this item (§17.5.5)
  created_at     TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX idx_quote_lines_quote ON quote_lines(quote_id);

-- ============ SUPPLIERS (wholesale partners — §17.5.10) ============
...
CREATE TABLE suppliers (
  ...
  -- v1.10 zero-risk referral pilot (§17.5.13): the first suppliers are 2–3 SMALL/MEDIUM RETAIL
...
-- ============ MATERIAL CATALOG (the Fixly price book — §17.5.6 L2, §17.5.10) ============
...
-- Prices are REFRESHED MONTHLY from supplier contracts (§17.5.10) — Jordan material prices move.
CREATE TABLE material_catalog (
  ...
  -- v1.10: cadence and confidence are per-item, set by which source actually exists (§17.5.14).
  ...
  base_reference_fils INTEGER,                      -- v1.12: the anchor the index formula multiplies (§17.5.14)
  ...
  brand              VARCHAR(60),                   -- v1.9: brand is a CUSTOMER choice, never the tech's (§17.5.11)
  ...
  variance_alert_bps INTEGER NOT NULL DEFAULT 1500, -- v1.9: >15% over reference ⇒ auto-escalate to review (§17.5.12)
...
-- ============ SERVICE RATE CARDS (tiered unit rates — §17.5.11) ============
...
-- ============ BILL OF MATERIALS (per booking, BEFORE execution — §17.5.12) ============
...
CREATE TABLE booking_materials (
  ...
  -- v1.10 variance dispute protocol (§17.5.15). Above variance_justify_bps (~20%) the technician
...
-- The five mandatory Materials-Policy questions (§17.5.4), answered per service and enforced
...
-- ============ SUPPLIER PRICE OBSERVATIONS (Amman retail price-refresh loop — §17.5.10) ============
...
-- ============ MATERIAL VERIFICATION REQUESTS (price-dispute resolution — §17.5.15) ============
...
-- ============ PRICE INDEX READINGS (official Jordanian sources — §17.5.14) ============
...
-- ============ CATEGORY READINESS GATE (quantitative L1→L2 trigger — §17.5.6) ============
...
INSERT INTO services (slug,name_ar,name_en,pricing_model,base_price_fils,inspection_fee_fils,materials_policy,est_duration_min,is_active,sort_order) VALUES
 ('electricity','كهرباء','Electricity','fixed_scope',50000,NULL,'labour_only',60,true,1),
 ('plumbing','سباكة','Plumbing','fixed_scope',40000,NULL,'labour_only',60,true,2),
 ('ac','تكييف','AC Cleaning','fixed_scope',30000,NULL,'materials_included',45,true,3),
 ('painting','دهان','Painting','quote_first',NULL,10000,'materials_quoted',180,false,4),
 ('furniture','أثاث','Furniture Assembly','fixed_scope',35000,NULL,'labour_only',90,false,5);

-- Painting price-book starter rows (tiers + coverage assumptions + caps — §17.5.6)
...
-- (all-in: labour + materials). Rates bracket the observed Jordan market (§17.5.11):
...
-- v1.9 readiness gate — Painting stays closed until the three thresholds are met (§17.5.6)
...
| `GET /bookings/{id}/materials` | View the Bill of Materials | per-line brand, qty, price, and the disclosed technician margin — visible **before** work starts (§17.5.12). Lines that are not `confirmed` are labelled as estimated, subject to on-site confirmation (§17.5.14) |
...
| `POST /bookings/{id}/materials/{lineId}/ack-variance` | Consent to an over-reference price | shows **reference vs requested** side by side; sets `customer_ack_at`. Declining opens a verification request (§17.5.15) |
...
| `POST /tech/bookings/{id}/materials/{lineId}/substitute` | Propose a substitution | same-or-higher tier only, function/compatibility/safety preserved; creates a linked line (`replaces_line_id`); any price increase re-enters customer approval (§17.5.13b) |
...
| `POST /tech/quotes/{id}/lines` | Draft itemized quote lines (quote-first) | `[{ kind, materialId?, description, qty, unit, unitPriceFils, suppliedBy }]` — material lines must reference `material_catalog` and price within its band; `422` on cap violation. Quote moves to `quoted` only after ops review when required (§17.5.8) |
...
| `GET /admin/quotes/review` | Ops-review queue for quotes (quote-first) | quotes over threshold / off-catalogue material lines (§17.5.8) |
...
| `GET /admin/materials` + `POST /admin/materials` | Manage the material price book | tiers, brands, wholesale basis, `[min,max]` caps, coverage notes, monthly refresh stamp (§17.5.10) |
...
| `GET /admin/categories/readiness` | Category readiness dashboard | per `quote_first` service: `quotes_closed/quotes_required`, `dispute_bps`, `price_deviation_bps`, `state` (§17.5.6) |
| `GET /admin/suppliers` + `POST /admin/suppliers` | Supplier records | pilot shops (no contract) + 30-day trial verdict fields; contracted wholesalers at Stage B (§17.5.10, §17.5.13) |
| `GET /admin/verifications` | Open price-verification requests | with the 24-hour countdown; overdue ⇒ auto-deduction (§17.5.15) |
...
| `POST /admin/price-index` | Record the monthly index readings | `{ kind, periodMonth, value, sourceUrl }` — the 15-minute ritual (§17.5.14) |
...
- **Queues / scheduled jobs (BullMQ):** `outbox-relay` (~1s), `dispatch-timeout` (90s → expand radius → `expired` + auto-void), `scheduled-dispatch`, `preauth-reconciler`, `balance-reconcile` (nightly), `location-pruner`, **`trust-tier-recompute`** (nightly — §0.2 #1), **`subscription-biller`** (monthly recurring charges — Phase 2), **`inspection-scheduler`** (subscriber quarterly free inspection — Phase 2), **`bom-review-timeout`** (2-hour founder-fallback: auto-approve capped at `price_max_fils`, audited — §0.6.2), **`verification-deadline`** (24-hour price-dispute settlement: no invoice ⇒ ledger deduction keyed `matverif:{id}` — §17.5.15), **`price-refresh-reminder`** (monthly ritual prompt + staleness report against `refresh_cadence` — §17.5.14), **`index-rebase`** (applies the CPI/fuel formulas to `base_reference_fils` once a new `price_index_readings` row lands; stamps `indexed_at` — §17.5.14), **`readiness-recompute`** (category gate after every closed quote — §17.5.6).
...
### 8.12 Bill of Materials & Price-Variance Gate (§17.5.10, §17.5.12)
...
### 17.5 Pricing & Materials Model (the complete pricing spec — v1.7 principles · v1.9 engine · v1.10 operations)
...
#### 17.5.2 Fixed-scope pricing (unchanged mechanics, restated)
...
5. **Ops review gate** (§17.5.8) where triggered → quote goes to the customer.
...
7. Any post-agreement discovery routes through the **extra-work gate** (§17.5.7) — never a silent price change.
...
#### 17.5.4 Materials Policy — the five mandatory questions

Every service category must answer these five questions **explicitly**, enforced by a **`service_material_policies` row per service** (v1.9 — the coarse v1.7 `services.materials_policy` flag is superseded by its 5-class `material_mode`); "unspecified" is not an allowed state:
...
| 3 | **Who buys** the materials? | See sourcing models §17.5.5 | Technician sources at catalogue price (Model 3 pricing control), platform-supplied later |
...
| **L3 — Supplier-integrated** | Wholesale supply contracts backing catalogue prices; monthly price refresh; optionally **Fixly Direct Supply** (§17.5.10) | Begins as soon as the first supply contract is signed; not required for MVP |
...
#### 17.5.7 Quote vs extra work — the boundary (why #3 doesn't cover #4)
...
#### 17.5.8 Six anti-manipulation controls
...
#### 17.5.9 The category-differentiated customer promise
...
#### 17.5.10 The Fixly Material Price Engine (v1.9 — Jordan-specific)

> §17.5.1–§17.5.9 established *that* materials must be governed. This subsection specifies *how*, against the actual Jordanian market. It is the difference between a policy and a mechanism.
...
4. **Transparency instead of ownership — this is the actual anti-padding mechanism.** Because Fixly owns no stock and therefore cannot *control* price, its one free lever is **mandatory proof**: every material purchase requires an uploaded **official shop invoice**; the app compares invoice price against the reference price; a gap beyond the allowed threshold **auto-rejects** pending explanation; and the **customer sees the real invoice photo in the app before final payment** (§17.5.12). Information plus a proof obligation closes invoice inflation **without spending a fils** — which is precisely why this model survives the capital constraint while a supply-chain model would not.
5. **Everything else is enforcement:** mandatory pre-execution BOM, variance auto-escalation, customer-chosen brand, locked-at-approval (§17.5.12).

**Stage B — participate in supply (L3; only once contracts exist).**

5. **Direct wholesale supply contracts.** Contracting accredited wholesalers (building-materials wholesalers of the Jordan Central / Al-Muqabalain type, and category equivalents for paint, electrical, plumbing) upgrades the price basis from *observed retail* to a **true wholesale cost** (`material_catalog.wholesale_fils`, `catalog_source = 'supplier_quote'`). *(Founder-owned commercial task, not engineering — §17.5.13.)*
...
#### 17.5.11 Per-category pricing mechanics
...
| **Plumbing** | Wide spread by part type and source | **Unified pricing per approved part class** from partner suppliers; parts outside the catalogue require the off-catalogue path (§17.5.12). |
...
#### 17.5.12 Bill of Materials — the anti-manipulation mechanism
...
#### 17.5.13 Rollout decision — pilot one category first
...
> **⚠️ v1.11 — the founder's answer was "all categories" (لكل الفئات), not a single-category pilot.** That decision is recorded here as the intent. It is **affordable under the Asset-Light model** precisely because coverage now costs *time* (collecting already-public price lists) rather than *capital* (per-category supply contracts) — the objection that made a narrow pilot necessary does not apply once nothing is being purchased. The remaining constraint is founder attention (§0.6), so the honest reconciliation is: **build the reference catalogue for all categories, but switch each one on in the app only as its data reaches usable coverage**, with `category_readiness_gate` (§17.5.6) deciding the order rather than a guess. Breadth of *data* is cheap; breadth of *live promises* is not.

- **Sequencing within an all-category build: Electrical first** (parts are discrete and countable, so its reference data is the fastest to assemble and the easiest to verify; brand-choice is also the sharpest demonstrable control). Painting follows as the first *`quote_first`* category once its readiness gate passes (§17.5.6).
...
| Only after a passed trial does anything get signed | The wholesale contracts of Stage B (§17.5.10) are the *reward* for a proven loop, not its precondition |
...
#### 17.5.9b Regional benchmarks and the hybrid pricing decision
...
| **Benna (بنّا)** | Building-materials platform built on **price comparison and access to trusted suppliers** | Price transparency is itself the product — consistent with Fixly's Asset-Light reference catalogue (§17.5.10) |
...
**Fixly must own materials *pricing* even before it owns materials *supply*.** Leaving estimation, purchasing and pricing entirely to the technician returns the customer to the traditional market and breaks the transparency promise. The phasing is: **Phase 1 — Fixly defines and prices materials; purchase happens through the technician or an approved shop, never at the technician's sole discretion. Phase 2 — a supplier network for the recurring items (paint, boxes, breakers, pipes, siphons, mixers, connectors). Phase 3 — direct procurement / assisted fulfilment for the highest-frequency categories.** *(Phases 2–3 require capital Fixly does not have — see §17.5.10; Phase 1 is the live model.)*
...
#### 17.5.13a The Jordanian materials catalogue — real items, real bands, real shops
...
#### 17.5.13c Per-category materials handling — simple vs. project work
...
#### 17.5.13b Materials edge-case register — the eight cases that decide clarity or chaos
...
#### 17.5.14 Keeping the price book honest — sources, cadence, and disclosed confidence (v1.10)
...
#### 17.5.15 Price-variance dispute protocol (v1.10 — the escalation that actually resolves)

§17.5.12's variance alert flags a suspicious price. This subsection specifies what *happens next* — the previously missing step, and the sharpest fraud surface in the whole model. It is a **decision path with an end state**, not a notification:

| Step | Trigger | What the system does |
|---|---|---|
| **1. Justify** | Entered price exceeds the reference by more than **`variance_justify_bps` (default 20%)** | The technician **cannot complete the invoice** until a **reason** is recorded (`variance_reason`: special type · imported brand · access difficulty · other, plus a note). *(A lower `variance_alert_bps` (~15%) still routes the line to review — §17.5.12; 20% additionally demands a justification and explicit customer consent.)* |
...
- **BOM / price-variance review queue (v1.9)** — material lines `pending_review` (>15% over catalogue reference, or off-catalogue), each actionable in one tap from a mobile deep link, with the 2-hour auto-resolution countdown visible (§0.6.2, §17.5.12).
- **Category readiness readout (v1.9)** — per `quote_first` category: closed quotes / 50, dispute %, price deviation %, and a plain ready/not-ready verdict (§17.5.6).
- **Stale-price warning (v1.9)** — catalogue rows past their `refresh_cadence`, plus anything flagged `estimated`/`under_review`, so the price book cannot silently drift from the market (§17.5.10, §17.5.14).
- **Open price-verification requests (v1.10)** — disputed lines with their 24-hour countdown and the amount at stake; overdue items settle automatically against the technician's dues (§17.5.15).
- **Supplier trial board (v1.10)** — the 2–3 pilot shops with their 30-day verdicts: commission actually paid? price manipulated in person? (§17.5.13).
...
| **Material price deviation** (v1.9) | \|final − estimated\| / estimated — proves the price book, not guesswork, sets prices; gates category expansion (§17.5.6) | < 15% |
| **BOM variance-escalation rate** (v1.9) | Share of material lines tripping the >15% alert — a rising number means the catalogue is stale or a technician is testing the limits (§17.5.12) | low + falling |
...
*Superseded (v1.9):* an earlier external review noted that this design is senior/staff-level (hexagonal architecture, outbox, optimistic locking, geo-dispatch, Skip transpiler) and therefore needed an experienced engineer to hit a 14-week target. **That target and staffing assumption are withdrawn** — the delivery model is now a solo founder with Claude Code on the ~7-month timeline in §0.6.4. The architectural sophistication remains a genuine execution risk to respect (it is why §0.6.1 gates the surface rather than the schema), but it is no longer answered by hiring.
...
| **2. Category expansion** | Months 6–9 | Add Furniture (`fixed_scope`, already seeded) + Painting (`quote_first`) | per-category complaint/redo rate within target before switching each on. **Painting hard gate (v1.7):** additionally requires the quote engine (`booking_quotes` + `quote_lines`), the Materials Policy answered (§17.5.4), and the `material_catalog` price book at maturity **L2** (§17.5.6) to be live — this moves video pre-check quotes from "Phase 2 nice-to-have" to **the prerequisite for material-heavy category expansion** |
...
### 17.19 Residual risk register — what this design does *not* close (v1.10)
...
| **Steel / price-critical inputs** | Classified as declared estimates with frozen fallback (§17.5.14) | This is a fix **by disclosure, not by accuracy** — the customer still sees a figure that is honestly labelled uncertain in this class |
...
- **Collusion between a technician and a supplier** to inflate an invoice that is internally consistent at every checkpoint — the invoice-photo step in §17.5.15 verifies a document, and a colluding pair can produce a genuine one;
