  ...
  return (
    <main className="max-w-[800px] mx-auto px-6 py-10">
      <div className="flex items-center gap-2">
        <BookOpen size={26} color="#1366D6" aria-hidden="true" />
        <h1 style={{ fontWeight: 800, fontSize: 28 }}>المدوّنة</h1>
      </div>
      <p className="mt-2" style={{ color: '#475569', fontSize: 14 }}>نصائح صيانة منزلية من فريق Fixly.</p>

      <div className="mt-6 space-y-3">
        {POSTS.map((p) => (
          <Card key={p.title} className="p-5">
            <div style={{ fontWeight: 700, fontSize: 16 }}>{p.title}</div>
            <div className="mt-1.5" style={{ color: '#475569', fontSize: 13, lineHeight: 1.6 }}>{p.excerpt}</div>
            <div className="mt-2" style={{ color: '#94A3B8', fontSize: 12, fontFamily: 'Inter' }}>{p.date}</div>
          </Card>
        ))}
      </div>
    </main>
